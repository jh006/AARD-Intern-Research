# -*- coding: utf-8 -*-
"""DNN_Carbon disulfide Infrared Spectrum

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1TZRSS1ivAKLZLBSY2AVj5QGY5NI5v1tk
"""
...
#! gdown --id 1LeC8mmdlNSm7FqVa4poM8O9-3Ruuq0D3
#df4 = pd.read_csv("spectrum.csv",header=0)
# https://webbook.nist.gov/cgi/cbook.cgi?ID=C75150&Units=SI&Type=IR-SPEC&Index=1#IR-SPEC
! gdown --id 1-h41TCbmEK0oooeibXYNxuGiuAufqybi
#https://drive.google.com/file/d/1-h41TCbmEK0oooeibXYNxuGiuAufqybi/view?usp=sharing
df4 = pd.read_csv("spectrum_02.csv",header=0)
print(df4.head()) #gives first 5 rows
print(df4.shape) #prints number of rows

spetrum=torch.tensor(df4.iloc[0:len(df4)].values).float().cuda()
print(spetrum)
print(spetrum[:,2])
print(spetrum.shape)

Transmittance =spetrum[:,1] 
frequency=spetrum[:,2]
frequency_np=frequency.cpu().numpy()   #to numpy
Transmittance_np=Transmittance.cpu().numpy()   #to numpy
 
center_f =np.arange(30, 60.1, 0.1) #center frequency
sinf=np.interp(center_f,frequency_np,Transmittance_np) #interpolated value, size=301
sinf_full=torch.tensor(sinf).float().cuda()   #full  sinf
sinf_full=torch.reshape(sinf_full, (1,301))
print(sinf_full)


#x_df = pd.DataFrame(sinf_full)
#x_df.to_csv('sinf.csv')
#sinf_full=sinf_full/sinf_full.sum(0).expand_as(sinf_full)
fig, ax = plt.subplots(figsize=(10, 10))
plt.plot(center_f,sinf, '-o')  # the ith row of all real data
plt.xlabel('Frequency')
plt.ylabel('Interpolated Transmittance')
#plt.legend(['Real','Predicted Real'])
 
plt.title("urea spectrum")
plt.show()

#use the 4 parameters in figure 6 para=torch.tensor([21.98,1.48,1.45,0.68]).float().cuda()  #page 3203 sinf band target   
...
#define the function to train the model using simulated sinf band data
def DNN_train(start_epoch, num_epoch, DNN, optimizer, checkpoint_path, best_path):
    ...
    low=torch.tensor([12,0.1,0.5,0.1]).cuda() # the lower limits of the four parameters
    #low=torch.tensor([12,0.5,1,0.5]).cuda() # the lower limits of the four parameters
    ...
    for epoch in range(start_epoch, num_epoch+start_epoch):
        optimizer.zero_grad()    
        x_train =Variable(sinf_full)   #simulated sinf band target spectra
        ...
        sinf_pred= torch.sqrt(torch.mul(pred_real, pred_real) +torch.mul(pred_imag, pred_imag)) # amplitude of designed spetra
        loss1=loss_func(sinf_full,sinf_pred)
        ...
        sinf_pred_np=sinf_pred.detach().cpu().numpy().flatten()
        sinf_full_np=sinf_full.detach().cpu().numpy().flatten()
        r2=r2_score(sinf_pred_np, sinf_full_np) 
        r_squared.append(r2)
...
if (start_epoch==1):
    ...
    start_epoch=1 
    num_epoch=1
    current_checkpoint="/content/drive/MyDrive/current_checkpoint_DNN_spectrum_02.pth" 
    best_checkpoint="/content/drive/MyDrive/best_checkpoint_DNN_spectrum_02.pth" 
...
print(torch.argmin(torch.FloatTensor(Trainloss))) #find the index of the smallest loss
print(r_squared[174730])
print(Trainloss[174730])
print(torch.argmax(torch.FloatTensor(r_squared))) #find the index of the largest r_square
print(r_squared[40245])
print(Trainloss[40245])

#Apply the best trained model to the single band data
best_checkpoint="/content/drive/MyDrive/best_checkpoint_DNN_spectrum_02.pth" 
...
with torch.no_grad(): # stop updating the parameters
   DNN_pred = DNN(sinf_full)
   pred_real=PNN_real(DNN_pred)
   pred_imag=PNN_imag(DNN_pred)
   sinf_pred = torch.sqrt(torch.mul(pred_real, pred_real) +torch.mul(pred_imag, pred_imag)) 
 
low=torch.tensor([12,0.1,0.5,0.1]).cuda()
up=torch.tensor([25,1.55,1.55,1.05]).cuda() 
DNN_pred_clipped=torch.clamp(DNN_pred, min=low, max=up)
loss_1=loss_func(sinf_pred,sinf_full.cuda())
...
sinf_pred2=sinf_pred.detach()
sinf_full2=sinf_full.detach()
...
lr=1e-1
sinf_pred_np=sinf_pred.cpu().numpy()
sinf_full_np=sinf_full.cpu().numpy()
y = sinf_full_np  
y_p=sinf_pred_np  
...
plt.title('DNN Model Loss_Epoch=%i'%k )
...
plt.ylabel('LOSS1')
plt.legend(['LOSS1'])
plt.title('DNN Model Loss1_Epoch=%i'%k )
#plt.savefig("Model Loss_Epoch="+str(k)+ ".png")
pdf.savefig(fig)
plt.show()
...
plt.ylabel('LOSS2')
plt.legend(['LOSS2'])
plt.title('DNN Model Loss2_Epoch=%i'%k )
pdf.savefig(fig)
plt.show()
...
plt.plot(r_squared[0:k])
plt.xlabel('Epoch')
plt.ylabel('R-square')
plt.title('DNN Model R-square_Epoch=%i'%k )
#plt.savefig("Model Loss_Epoch="+str(k)+ ".png")
pdf.savefig(fig)
plt.show()
